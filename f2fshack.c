...
struct f2fs_sit_entry* lookup_sit(unsigned int ori_pbn){
    ...
    // if(flag==0){
    //     //flag =1;
    //     printf("f2fs_start:%p, segno:%ld, block_index:%ld, seg_index:%ld, block_no:%ld entry:%p\n", f2fs_start, segno, block_index, seg_index, block_no, entry);
    //     printf("vblocks:%d\nmap:",entry->vblocks);
    //     for(int i=0;i<16;i++)
    //         printf("%u ",entry->valid_map[i]);
    //     printf("\nmtime:%ld\n", entry->mtime);
    // }
    return entry;
}
...
int open_device(char* f2fs_img){
    ...
    assert(cp_e->checkpoint_ver==cp->checkpoint_ver);
    nat_bitmap = &(cp->sit_nat_version_bitmap[cp->sit_ver_bitmap_bytesize]);
    sit_bitmap = &(cp->sit_nat_version_bitmap);
    printf("vaild_block_count:%ld\n",cp->valid_block_count);
    return 0;
}

void close_with_sync(){
      u32 chksum = f2fs_checkpoint_chksum(cp);
      printf("chksum:%u\n",chksum);
      u32* crc = (u32*)((void*)cp + cp->checksum_offset);
      *crc = chksum; 
      memcpy(cp_e, cp, F2FS_BLKSIZE);
      printf("vaild_block_count:%ld\n",cp->valid_block_count);
